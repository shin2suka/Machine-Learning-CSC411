...
class Model(object):
    ...
    def update_pi(self, R):
        ...
        ######################## Your code here #########################
        K = R.shape[1]
        denominator = np.sum(R) + K * (self.prior.a_mix - 1)
        numerator = np.sum(R, axis=0) + (self.prior.a_mix - 1)
        self.params.pi = np.divide(numerator, denominator)
    ...
    def update_theta(self, X, R):
        ...
        ######################## Your code here #########################
        numerator = np.dot(np.transpose(R), X) + self.prior.a_pixels - 1
        denominator = np.sum(R, axis=0) + self.prior.a_pixels + self.prior.b_pixels - 2
        self.params.theta = numerator/denominator.reshape(R.shape[1], 1)
...
def print_part_2_values():
    ...
    model = train_from_labels(show=True)
...
if __name__ == '__main__':
    print_part_1_values()
    print_part_2_values()
    model1 = train_from_labels()
    model2 = train_with_em()
    print_log_probs_by_digit_class(model1)
    print_log_probs_by_digit_class(model2)
